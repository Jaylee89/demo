; (function($) {
	...
	$('.us_panel_apply').click(function() {
		var logined = 1;
		if (logined == 1) {
			$('#add_apply').addClass('show');
		} else {
			$('#reg_index').addClass('show')
		}
	});	
	
	$('.us_panel_collect').click(function() {
		if (logined != 1) {
			$('#reg_index').addClass('show');                
		}
	});	
	
	$('#add_apply .hd .fr').click(function() {
		$('#add_apply').removeClass('show')
	});
	
	$('.note-tip').click(function() {
		if($(this).hasClass('on') == false ){
			if($('.act-user').hasClass('show') == true){
				$('.act-user').hide().removeClass('show');
				$('.note-list').show();
						
				}else{
				$('.note-list').hide();
				$('.act-user').show().addClass('show');
				}
			$('.note-tip').removeClass('on');
			$(this).addClass('on');
		}
	});
	
	$('.note-list li').click(function(){
		if($(this).find('.txt-hide').hasClass('show') == false ){
			
			$(this).find('.txt-hide').show().addClass('show');
			}else{
			$(this).find('.txt-hide').removeClass('show').hide();
				}
		})
